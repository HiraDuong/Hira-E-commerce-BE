...
class MerchandiseController {
    ...
    async createMerchandise(req: Request, res: Response) {
        ...
        try {
            ...
            // check user_id match
            if ((req as CustomRequest).token.user_id.toString() !== shop.user_id.toString()) {
            ...
            const new_merchandise = new Merchandise();
                ...
                new_merchandise.merchandise_price = req.body.merchandise_price,
                new_merchandise.merchandise_description = req.body.merchandise_description || '',
                new_merchandise.shop_id = req.body.shop_id,
                new_merchandise.merchandise_in_stock_quantity = req.body.merchandise_in_stock_quantity,
    ...
    async updateMerchandiseById(req: Request, res: Response) {
        ...
        try {
            ...
            const merchandise = await new MerchandiseRepo().findById(req.params.id);
            if (!merchandise) {
                return res.status(404).json({
                    status: "Not Found!",
                ...
                merchandise.merchandise_name = req.body.merchandise_name || merchandise.merchandise_name,
                merchandise.merchandise_price = req.body.merchandise_price || merchandise.merchandise_price,
                merchandise.merchandise_description = req.body.merchandise_description || merchandise.merchandise_description,
                merchandise.merchandise_in_stock_quantity = req.body.merchandise_in_stock_quantity || merchandise.merchandise_in_stock_quantity,
                merchandise.merchandise_img = req.body.merchandise_img || merchandise.merchandise_img
    ...
    async getMerchandiseByShopId(req: Request, res: Response) {
        try {
            const merchandises = await new MerchandiseRepo().findByShopId(req.params.shop_id);
    ...
    async getMerchandiseByName(req: Request, res: Response) {
        try {
            const merchandises = await new MerchandiseRepo().findByName(req.params.name);
            if (merchandises.length === 0) {
                return res.status(404).json({
                    status: "Not Found!",
                    message: "Merchandises not found!",
