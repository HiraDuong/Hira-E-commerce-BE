...
class ShopController {
    ...
    async createShop(req: Request, res: Response) {
        if((req as CustomRequest).token.user_id.toString() !== req.body.user_id.toString()) { 
            return res.status(403).json({
                status: "Forbidden!",
                message: "You are not allowed to create a shop for another user!"
            });
        }
        try {
    ...
    && (req as CustomRequest).token.user_role.toString()!== 'admin'
            ...
            return res.status(403).json({
                status: "Forbidden!",
                message: "You are not allowed to delete a shop for another user!"
            });
        }
        try {
    ...
    async updateShop(req: Request, res: Response) {
        ...
        try {
            ...
            // check authorization
            if((req as CustomRequest).token.user_id.toString() !== updated_shop?.user_id.toString()){
                return res.status(403).json({
                    status: "Forbidden!",
                    message: "You are not authorized to update this shop!",
                });
            }
            updated_shop.shop_name = req.body.shop_name || updated_shop.shop_name;
            updated_shop.shop_description = req.body.shop_description || updated_shop.shop_description;
            updated_shop.shop_avt = req.body.shop_avt || updated_shop.shop_avt;
            updated_shop.shop_start_at = req.body.shop_start_at || updated_shop.shop_start_at;
            updated_shop.shop_address = req.body.shop_address || updated_shop.shop_address;
    ...
    async getAllShopsByShopName(req: Request, res: Response) {
        try {
            const shops = await new ShopRepo().findByShopName(req.params.shop_name);
            res.status(200).json({
                status: "Success!",
                message: "Successfully fetched shops!",
                data: shops
